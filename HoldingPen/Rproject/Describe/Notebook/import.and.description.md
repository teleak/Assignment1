...
### two cleaned data sets

#### loans = data with NAs removed, df 'data' (kept NA values)

```r
nrow(has.na)  ### number of rows with NA values
```
...
```r
loans <- data[(complete.cases(data)), ]  ### rows with NA values removed
```
...
```r
write.csv(loans, "~/Desktop/RCourse/DataAnalysis/AnalysisAssignment1/Rproject/ProcessedData/Working/loans.csv")
write.csv(data, "~/Desktop/RCourse/DataAnalysis/AnalysisAssignment1/Rproject/ProcessedData/Working/data.csv")
```
